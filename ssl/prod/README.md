...
```
```
docker compose -f docker-compose.prod.yml run --rm certbot certonly --webroot --webroot-path /var/www/certbot/ -d *.bln.projecteats.org
```
```
docker compose -f docker-compose.prod.yml run --rm certbot certonly --dns-route53 -d *.bln.projecteats.org --dry-run
```
